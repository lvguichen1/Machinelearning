...
def draw_america():
    ...
    m.drawcoastlines(linewidth=0.3, antialiased=False, color='#303030')
    m.drawcountries(linewidth=1, linestyle='-', antialiased=False, color='k')
    m.drawstates(linewidth=0.5, linestyle='--', antialiased=True, color='k')
    m.drawparallels(np.arange(0, 90, 10), labels=[True, True, False, False])  # 绘制纬线 [left,right,top,bottom]
